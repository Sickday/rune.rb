...
module Readable
  ...
  def read(type = :byte, opt = { signed: false, mutation: :STD, order: :BIG,})
    ...
    when :socket, :io then read_from_io(opt[:io], opt[:length])
    end
  end
  ...
  # @param io [IO, TCPSocket] the io to read from
  # @param length [Integer] the length of payload to read.
  def read_from_io(io, length)
    @readable = NIO::ByteBuffer.new(length || @header[:length])
    @readable.read_from(io)
    @readable.flip
  end
end