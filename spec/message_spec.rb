...
describe RuneRb::Network::Message do
  describe '#new' do
    it 'constructs a new {RuneRb::Network::Buffer} with the passed body parameter' do
      junk_data = JUNK_DATA_FACTORY.call
      stub_message = RuneRb::Network::Message.new(body: junk_data)
      expect(stub_message.body.class).to eql(RuneRb::Network::Buffer)
    ...
    it 'constructs a new {Message#header} struct with the passed op_code, length, and type' do
      rand(0xff).times do |itr|
        junk_data = JUNK_DATA_FACTORY.call
        stub_message = RuneRb::Network::Message.new(op_code: itr, body: junk_data)
    ...
    it 'raises a {StandardError} if a body parameter is passed whose type is not {RuneRb::Network::Buffer}, {StringIO}, or {String}' do
      expect { RuneRb::Network::Message.new(body: :SickoMode) }.to raise_error(StandardError)
    end
  end
  ...
  describe '#compile' do
    ...
    it 'excludes the {Message#header#op_code} and {Message#header#length) if the {Message#type} is :RAW' do
      junk_data = JUNK_DATA_FACTORY.call
      stub_message = RuneRb::Network::Message.new(body: junk_data, type: :RAW)
    ...
    it 'excludes the {Message#header#length if the {Message#type} is :FIXED' do
      junk_data = JUNK_DATA_FACTORY.call
      stub_message = RuneRb::Network::Message.new(body: junk_data, type: :FIXED)
      out = stub_message.header.compile_header
      msg_out = stub_message.compile
