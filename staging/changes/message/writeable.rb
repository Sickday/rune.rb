module Writeable
  ...
  def compile(type = :FIXED)
    write_header(type)
    puts "Writing content for: #{@header.inspect}"
    @writeable
  end
  ...
  # Write the compiled header to the head of the writable payload.
  # @param type [Symbol] the type of header to compile for the message. (:FIXED, :VARIABLE_BYTE, :VARIABLE_SHORT)
  def write_header(type)
    @writeable.prepend(@header.compile_for(@writeable, type))
  end
end