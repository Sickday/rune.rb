...
on_item_option(3415) { |player, id, slot| Trade.remove_item(player, slot, id, 1) }

# Item offer/remove amount 5
on_item_option2(3322) { |player, id, slot| Trade.offer_item(player, slot, id, 5) }
on_item_option2(3415) { |player, id, slot| Trade.remove_item(player, slot, id, 5) }

# Item offer/remove amount 10
on_item_option3(3322) { |player, id, slot| Trade.offer_item(player, slot, id, 10) }
on_item_option3(3415) { |player, id, slot| Trade.remove_item(player, slot, id, 10) }

# Item offer/remove amount all
on_item_option4(3322) { |player, id, slot| Trade.offer_item(player, slot, id, player.inventory.count(id)) }
on_item_option4(3415) { |player, id, slot| Trade.remove_item(player, slot, id, player.offered_items.count(id)) }

# Item offer/remove amount x
on_int_enter_amount(3322) { |player, id, slot, amount| Trade.offer_item(player, slot, id, amount) }
on_int_enter_amount(3415) { |player, id, slot, amount| Trade.remove_item(player, slot, id, amount) }

##
# Close Trade button.
on_int_close(3323) { |player| Trade.close(player) }
on_int_close(3443) { |player| Trade.close(player) }

##
# Request/Accept buttons.
on_player_trade(153) { |player, packet| Trade.request(player, packet) }
on_player_trade(139) { |player, packet| Trade.accepted_request(player, packet) }
...
class Trade
  def self.request(player, packet)
    target_slot = packet.read_byte
    target = WORLD.players[target_slot - 1]
  ...
  def self.accepted_request(player, packet)
    ...
    if player.request_manager.request_state == :requested
      open_interface(player, target)
      player.request_manager.request_state = :participating
      target.request_manager.request_state = :participating
    end
  end

  def self.open_interface(player, target)
  ...
  def self.accepted_trade(player, target)
    cancel(player, target) if player.nil? || target.nil?
  ...
  def self.offer_item(player, slot, id, amount = 1)
    ...
    close player, target if player.nil? || target.nil?

    item = player.inventory.items[slot]
    return if item.nil?

    # Make sure we only add as many as we have
    amount = player.inventory.count item.id if amount > player.inventory.count(item.id)

    offeredItem = RuneRb::Item::Item.new item.id, amount
  ...
  def self.remove_item(player, slot, id, amount = 1)
    ...
    # Make sure we only add as many as we have
    amount = player.offered_items.count item.id if amount > player.offered_items.count(item.id)

    removedItem = RuneRb::Item::Item.new item.id, amount
  ...
  def self.mask_list(items)
    ...
    items.each do |i|
      ...
      if count == 0
        output = RuneRb::Item::ItemDefinition.for_id(i.id).name
      else
        output << "\\n#{RuneRb::Item::ItemDefinition.for_id(i.id).name}"
      end

      output << " x #{amount}" if RuneRb::Item::ItemDefinition.for_id(i.id).stackable
  ...
  def self.exchange(player, target)
    ...
    player.gained_items.items.each { |i| player.inventory.add(i) unless i.nil? }
    target.gained_items.items.each { |i| target.inventory.add(i) unless i.nil? }
  ...
  def self.cancel(player, target)
    # Reset player items
    unless player.nil?
      player.offered_items.items.each { |i| player.inventory.add(i) unless i.nil? }
      clean(player)
    ...
    unless target.nil?
      target.offered_items.items.each { |i|
        target.inventory.add i unless i.nil?
      }
