[{id: 4079, hook: :item_click, anim: 1457}, # Play yo-yo
 ...
 {id: 6867, hook: :item_alt2, anim: 3005, graphic: 509, gdelay: 2}].each do |item|
  method("on_#{item[:hook]}").call(item[:id]) do |player, _slot|
    player.play_animation(RuneRb::Model::Animation.new(item[:anim], item[:adelay] || 0)) if item.include?(:anim)
    player.play_graphic(RuneRb::Model::Graphic.new(item[:graphic], item[:gdelay] || 0)) if item.include?(:graphic)
  end
end
