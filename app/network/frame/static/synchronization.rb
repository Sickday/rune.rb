module RuneRb::Net::Static
  class SynchronizationFrame < RuneRb::Net::MetaFrame
    ...
    def parse(context)
      ...
      context.session.write(:region, regional: context.position.regional) if context.flags[:region?]
      ...
      state_block = RuneRb::Net::Static::StateBlock.new(context)
    ...
    def write_locals(state, context, world)
      # Pre-removal setup. Ensure only players that actually exist in the world are in the context's list initially.
      context.locals[:players].delete_if { |player| !world.entities[:players].include?(player) }
      # Loop through all mobs in the World attached to the player to check if it should add Mobs to the context player's list.
      # TODO: This is a bad idea. When the world's population reaches large numbers this will take forever to do per-player. Do this by region instead.
      world.locals_for(context).each do |ctx|
        # Break if the list has reached it's max capacity
        break if context.locals[:players].size >= 255
        # Skip this iteration if the context is the context we're updating
        next if ctx == context

        # Check if the context needs removal if the context is already in the player's local list
        if context.locals[:players].include?(ctx)
          # Remove the context if it's teleporting or it's no longer in view
          if ctx.flags[:teleport?] || !ctx.position.in_view?(context.position)
            write_bits(1, 1)
            write_bits(2, 3)
          else
            puts 'Updating context'
            # Write the movement and state for the context if it requires a state update.
            write_movement(ctx)
            state.write_state(ctx)
          end
        elsif !ctx.flags[:teleport?] && ctx.position.in_view?(context.position) # Add the context if it does not exist in the list, is not teleporting, and is within view
          puts 'Adding context'
    ...
    def write_new_context(context, initial)
      ...
      write_bits(11, initial.world.entities[:players].index(context) + 1)
      ...
      write_bits(5, context.position[:y] - initial.position[:y])
      write_bits(5, context.position[:x] - initial.position[:x])
    end
    ...
    def write_placement(context)
      ...
      write_bits(2, context.position[:z]) # Write the plane. 0 being ground level
      write_bit(context.flags[:region?]) # Region change?
      write_bit(context.flags[:state?]) # Update State/Appearance?
      write_bits(7, context.position.local_x) # Local Y
      write_bits(7, context.position.local_y) # Local X
      log "Wrote [x: #{context.position.local_x}, y: #{context.position.local_y}]"
    end
    ...
    def write_walk(context)
      ...
      write_bits(3, context.movement[:primary])
      write_bit(context.flags[:state?])
    end
    ...
    def write_run(context)
      ...
      write_bits(3, context.movement[:primary])
      write_bits(3, context.movement[:secondary])
      write_bit(context.flags[:state?])
    end
  end
end