  ...
  class PlayerUpdateTask
    ...
    def execute
      ...
      WORLD.region_manager.get_local_players(@player).each do |p|
        ...
        @player.local_players << p ## ADD TO CONTEXT PLAYER'S LOCAL LIST
        add_new_player(packet, p) ## ADD NEW LOCAL PLAYER
        update_player(update_block, p, true, false) # UPDATE THE PLAYER
      end
    ...
    ##
    # TODO: This while "sprite" system for update bits needs a big fat refactor. It's needlessly confusing and it's tough to for anyone to understand why things are happening.
    def update_player_movement(packet, p)
      ...
      if sprites[0] == -1
        ...
        packet.add_bits(1, 1)
            .add_bits(2, 1)
            .add_bits(3, sprites[0])
            .add_bits(1, p.flags.update_required? ? 1 : 0)
      else
        packet.add_bits(1, 1)
            .add_bits(2, 2)
            .add_bits(3, sprites[0])
            .add_bits(3, sprites[1])
            .add_bits(1, p.flags.update_required? ? 1 : 0)
      end
    end
    ...
    def add_new_player(packet, p)
      packet.add_bits(11, p.index)
          .add_bits(1, 1)
          .add_bits(1, 1)
          .add_bits(5, p.location.y - @player.location.y)
          .add_bits(5, p.location.x - @player.location.x)
    end

    def append_forced_move(p, block)
      block.add_byte_s(p.location.get_local_x) # x1 (local)
          .add_byte_s(p.location.get_local_y) # y1 (local)
          .add_byte_s((p.location.get_local_x + 2)) # x2 (local)
          .add_byte_s(p.location.get_local_y) # y2 (local)
          .add_leshort_a(0) # speed to move from current to (x1, y1)
          .add_short_a(20) # speed to move from (x1, y1) to (x2, y2)
          .add_byte_s(-3) # direction
    end

    def append_graphics(p, block)
      block.add_leshort(p.graphic.id)
          .add_int(p.graphic.delay)
    end

    def append_animation(p, block)
      block.add_leshort(p.animation.id)
          .add_byte_c(p.animation.delay)
    end
    ...
    def append_chat(p, block)
      ...
      block.add_leshort((msg.color << 8).ubyte | msg.effects.ubyte)
          .add_byte(RuneRb::World::RIGHTS.index(p.rights))
          .add_byte_c(msg.text.size)
          .add_bytes(msg.text.reverse)
    end
