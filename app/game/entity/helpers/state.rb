module RuneRb::Game::Entity::Helpers::State
  ...
  def state_update?
    @flags[:chat?] || @flags[:graphic?] || @flags[:animation?] || @flags[:looks?]
  end
  ...
  def generate_looks
    @state.looks_buffer = RuneRb::Network::Buffer.new('w')
    # Gender
    ...
    # Player's name
    @state.looks_buffer.write(@profile.name_hash, type: :long)
    # Combat Level
    @state.looks_buffer.write(128, type: :byte)# profile.skills.combat_level, type: :byte)
    # Skill Level
    @state.looks_buffer.write(@profile.skills.values[:total_level], type: :short)
  end

  def generate_state
    @state.state_buffer = RuneRb::Network::Buffer.new('w')
    write_mask
